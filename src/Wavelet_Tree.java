...
public class Wavelet_Tree {
    ...
    public static int select(Node node, int occurrence, char ch) {
        ...
        boolean b2= n.getIsLeftChild();
        if (occurrence < 1 || occurrence > n.getIsInFirstHalf().size() || n.getParent() == null) {
            return -2;
        }
        return select(occurrence, n.getParent(), b, b2);
    }

    private static int select(int occurrence, Node node, boolean isLeftInRoot, boolean fromLeft) {
        int counter = 0;
        int index;
        ...
        for (index = 0; counter < occurrence; index++) {
            if (Objects.equals(fromLeft, node.getIsInFirstHalf().get(index))) {
                counter++;
            }
        }
        return select(index, node.getParent(), isLeftInRoot, node.getIsLeftChild());
